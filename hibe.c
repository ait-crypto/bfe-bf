...
int hibe_extract(hibe_private_key_t* privateKey, hibe_system_params_t* systemParams, ep_t masterKey,
                 const char* id) {
  int status = BFE_SUCCESS;
  bn_t order, r, two;
  ep_t a0, hTemp;
...
int hibe_derive(hibe_private_key_t* privateKey, hibe_system_params_t* systemParams,
                hibe_private_key_t* parentPrivateKey, const char* id) {
  int status = BFE_SUCCESS;
  bn_t order, t, two;
  ep_t a0, hTemp;
...
int hibe_encrypt(hibe_ciphertext_t* ciphertext, hibe_system_params_t* systemParams, const char* id,
                 uint8_t* message) {
  int status = BFE_SUCCESS;
...
hibe_setup_pair_t* hibe_init_setup_pair(int depth) {
  ...
  setupPair->systemParams =
      malloc(offsetof(hibe_system_params_t, h) + depth * sizeof(setupPair->systemParams->h[0]));
...
hibe_private_key_t* hibe_init_private_key(hibe_system_params_t* systemParams, const char* id) {
  ...
  hibe_private_key_t* privateKey =
      malloc(offsetof(hibe_private_key_t, b) + bLen * sizeof(privateKey->b[0]));
...
hibe_ciphertext_t* hibe_init_ciphertext(int messageLen) {
  hibe_ciphertext_t* ciphertext =
      malloc(offsetof(hibe_ciphertext_t, a) + messageLen * sizeof(ciphertext->a[0]));
