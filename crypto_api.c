...
int crypto_kem_keypair(unsigned char* serialized_pk, unsigned char* serialized_sk) {
  ...
  if (!status) {
    ...
    bfe_bf_secret_serialize(serialized_sk + CRYPTO_PUBLICKEYBYTES, &sk);
  }
...
int crypto_kem_enc(unsigned char* serialized_ct, unsigned char* k,
  ...
  int status = bfe_bf_public_deserialize(&pk, serialized_pk);
  if (status) {
    return status;
  }
...
int crypto_kem_dec(unsigned char* k, const unsigned char* serialized_ct,
  ...
  bfe_bf_public_key_t pk;
  // deserialize the public key
  int status = bfe_bf_public_deserialize(&pk, serialized_sk);
  if (status) {
    return status;
  }
  ...
  bfe_bf_secret_key_t sk;
  status = bfe_bf_secret_deserialize(&sk, serialized_sk + CRYPTO_PUBLICKEYBYTES);
  if (status) {
    goto ret;
  }
  ...
  status = bfe_bf_deserialize(&ct, serialized_ct);
  if (status) {
    goto ret;
  }
...
int crypto_kem_punc(unsigned char* serialized_sk, const unsigned char* serialized_ct) {
  // deserialize the secret key
  bfe_bf_secret_key_t sk;
  int status = bfe_bf_secret_deserialize(&sk, serialized_sk + CRYPTO_PUBLICKEYBYTES);
  if (status) {
    return status;
  }
  ...
  // deserialize the ciphertext
  bfe_bf_ciphertext_t ciphertext;
  status = bfe_bf_deserialize(&ciphertext, serialized_ct);
  if (status) {
    goto ret;
  }
