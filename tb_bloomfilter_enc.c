...
int _generate_time_keys(const char* currentNodeId, const char* targetNodeId,
                        hibe_private_key_t* currentNodeKey,
                        tb_bloomfilter_enc_secret_key_t* secretKey,
                        tb_bloomfilter_enc_system_params_t systemParams);
int _generate_bf_keys(const char* nodeId, hibe_private_key_t* nodeKey,
                      tb_bloomfilter_enc_secret_key_t* secretKey,
                      tb_bloomfilter_enc_system_params_t systemParams);
...
int tb_bloomfilter_enc_setup(tb_bloomfilter_enc_setup_pair_t* setupPair, int keyLength) {
  ...
  TRY {
    ...
    hibeSetupPair = hibe_init_setup_pair(setupPair->systemParams.timeSlotsExponent +
                                         _calculate_target_bloom_filter_tree_depth(
                                             bloomfilter_get_size(setupPair->secretKey->filter)));
    ...
    if (!status) {
      ...
      hibe_extract(setupPair->secretKey->key[0], hibeSetupPair->systemParams,
                   hibeSetupPair->masterKey, "0");
      setupPair->secretKey->key[setupPair->systemParams.timeSlotsExponent] =
          hibe_init_private_key(hibeSetupPair->systemParams, "1");
      hibe_extract(setupPair->secretKey->key[setupPair->systemParams.timeSlotsExponent],
                   hibeSetupPair->systemParams, hibeSetupPair->masterKey, "1");
  ...
  CATCH_ANY {
    logger_log(LOGGER_ERROR,
               "Error occurred in Time-Based Bloom Filter Encryption setup function.");
...
int tb_bloomfilter_enc_encrypt(tb_bloomfilter_enc_ciphertext_t* ciphertext,
  ...
  tb_bloomfilter_enc_encapsulated_keys_t* encapsulatedKeys =
      malloc(offsetof(tb_bloomfilter_enc_encapsulated_keys_t, keys) +
             systemParams.filterHashCount * sizeof(encapsulatedKeys->keys[0]));
  ...
  TRY {
    ...
    /*
    printf("\nOriginal key:\n");
    for (int i = 0; i < systemParams.keyLength; i++) {
        printf("%c", K[i]);
    }
    */

    int bitPositions[systemParams.filterHashCount];
    bloomfilter_get_bit_positions(bitPositions, ciphertext->c, systemParams.keyLength,
                                  systemParams.filterHashCount, systemParams.filterSize);
    ...
    for (int i = 0; i < systemParams.filterHashCount; i++) {
      ...
      strncpy(
          keyIdentity, intervalId,
          strlen(
              intervalId)); // instead of strlen we can also use time tree depth from system params
      if (bitPositions[i] > 0) { // because log2 is -infinity for 0
        itoa(bitPositions[i],
             &keyIdentity[strlen(intervalId) + bfTreeDepth -
                          ((int)floor(log2(bitPositions[i])) + 1)],
             2); // padding bit position binary representation and adding it to full key identity
      }
      encapsulatedKeys->keys[i] = hibe_init_ciphertext(systemParams.keyLength);
      status =
          hibe_encrypt(encapsulatedKeys->keys[i], systemParams.hibeSystemParams, keyIdentity, K);
    }
  ...
  CATCH_ANY {
    logger_log(LOGGER_ERROR,
               "Error occurred in Time-Based Bloom Filter Encryption encrypt function.");
    status = BFE_ERR_GENERAL;
  }
  FINALLY {
    bn_free(order);
...
int tb_bloomfilter_enc_decrypt(uint8_t* key, tb_bloomfilter_enc_system_params_t systemParams,
  ...
  int bitPositions[systemParams.filterHashCount];
  bloomfilter_get_bit_positions(bitPositions, ciphertext->c, ciphertext->cLen,
                                systemParams.filterHashCount, systemParams.filterSize);

  for (int i = 0; i < systemParams.filterHashCount; i++) {
    if (secretKey->key[bitPositions[i] + systemParams.timeSlotsExponent + 1] != NULL) {
      status = hibe_decrypt(key, ciphertext->encapsulatedKeys->keys[i],
                            secretKey->key[bitPositions[i] + systemParams.timeSlotsExponent + 1]);
      if (!status) {
...
void tb_bloomfilter_enc_puncture_key(tb_bloomfilter_enc_secret_key_t* secretKey,
  ...
  bloomfilter_get_bit_positions(affectedIndexes, ciphertext->c, ciphertext->cLen,
                                secretKey->filter.hashCount,
...
int tb_bloomfilter_enc_puncture_int(tb_bloomfilter_enc_secret_key_t* secretKey,
  ...
  if (newIntervalIndex > 0) {
    itoa(newIntervalIndex,
         &newIntervalBinaryIndex[systemParams.timeSlotsExponent -
                                 ((int)floor(log2(newIntervalIndex)) + 1)],
         2);
  }
  ...
  if (newIntervalBinaryIndex[strlen(newIntervalBinaryIndex) - 1] == '1') {
    ...
    _generate_time_keys(closestTimeKeyId, newIntervalBinaryIndex,
                        secretKey->key[closestTimeKeyIndex], secretKey, systemParams);
    timeKeyIndex = 0;
  }
...
int _generate_time_keys(const char* currentNodeId, const char* targetNodeId,
  ...
  TRY {
    hibe_private_key_t* leftNodeKey =
        hibe_init_private_key(systemParams.hibeSystemParams, leftNodeId);
    hibe_derive(leftNodeKey, systemParams.hibeSystemParams, currentNodeKey, leftNodeId);
    hibe_private_key_t* rightNodeKey =
        hibe_init_private_key(systemParams.hibeSystemParams, rightNodeId);
  ...
  CATCH_ANY {
    logger_log(
        LOGGER_ERROR,
        "Error occurred in Time-Based Bloom Filter Encryption time keys generation function.");
    status = BFE_ERR_GENERAL;
  }
  FINALLY {}
...
int _generate_bf_keys(const char* nodeId, hibe_private_key_t* nodeKey,
  ...
  int bfTreeDepth =
      _calculate_target_bloom_filter_tree_depth(bloomfilter_get_size(secretKey->filter));
  int fullTreeDepth = systemParams.timeSlotsExponent + bfTreeDepth;
  if (strlen(nodeId) == fullTreeDepth) {
    int secretKeyIndex =
        (int)strtol(&nodeId[systemParams.timeSlotsExponent], NULL, 2) +
        systemParams.timeSlotsExponent +
        1; // adding offset since the first part of the key is reserved for time keys
  ...
  CATCH_ANY {
    logger_log(LOGGER_ERROR,
               "Error occurred in Time-Based Bloom Filter Encryption BF keys generation function.");
    status = BFE_ERR_GENERAL;
  }
  FINALLY {
    hibe_free_private_key(nodeKey);
...
tb_bloomfilter_enc_init_ciphertext(tb_bloomfilter_enc_system_params_t systemParams) {
  tb_bloomfilter_enc_ciphertext_t* ciphertext =
      malloc(offsetof(tb_bloomfilter_enc_ciphertext_t, c) +
             systemParams.keyLength * sizeof(ciphertext->c[0]));
...
tb_bloomfilter_enc_setup_pair_t* tb_bloomfilter_enc_init_setup_pair(int filterElementNumber,
  ...
  returnPair->secretKey = malloc(offsetof(tb_bloomfilter_enc_secret_key_t, key) +
                                 (timeTreeDepth + 1 + (int)pow(2, bfTreeDepth)) *
                                     sizeof(returnPair->secretKey->key[0]));
  memset(returnPair->secretKey, 0,
         offsetof(tb_bloomfilter_enc_secret_key_t, key) +
             (timeTreeDepth + 1 + (int)pow(2, bfTreeDepth)) *
                 sizeof(returnPair->secretKey->key[0]));
  returnPair->secretKey->keyLen =
      timeTreeDepth + 1 +
      (int)pow(2, bfTreeDepth); // todo this casting is not really perfect way of dealing with this
